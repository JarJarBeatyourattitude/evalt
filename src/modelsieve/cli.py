"""Compatibility command for the former ModelSieve name."""
...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(prog="modelsieve", description="Optimize prompts and find the cheapest model that passes your approved examples.")
    ...
    optimize.add_argument("--output", default="modelsieve-result.json")
    return root
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "draft":
            draft = client.draft_answer(task=args.task, input=args.input, model=args.model, max_cost_usd=args.max_cost)
            print(json.dumps({
                "task": draft.task,
                "input": draft.input,
                "answer": draft.answer,
        ...
        print(f"modelsieve: {error}", file=sys.stderr)
        return 2
